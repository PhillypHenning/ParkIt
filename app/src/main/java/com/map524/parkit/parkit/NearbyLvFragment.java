...
public class NearbyLvFragment extends Fragment {
    ...
    public ArrayList<ParkingDataModel> parse_data(String[][] s){
        ...
        for(String[] item : s){
            ...
            //Double max_height = Double.parseDouble(item[9]); //changed Double for String on Max_height
            String max_height = item[9];

            data.add(new ParkingDataModel(id, lat, lng, item[4], max_height, item[1], item[6], item[7], item[10], item[11]));
        }
